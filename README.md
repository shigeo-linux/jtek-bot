...
> **Linux only.** Runs as a systemd user service on a Linux machine. The host must be running at 4:00 PM ET each day for the signal to fire (if the machine is off, `Persistent=true` ensures it runs at next boot).
